...
function _git_time_since_commit() {
  ...
  if last_commit=$(git log --pretty=format:'%at' -1 2> /dev/null); then
    ...
    if [[ $years -gt 0 ]]; then
      commit_age="${years} years $((days % 365 )) days ago"
    elif [[ $days -gt 0 ]]; then
      commit_age="${days} days $((hours % 24)) hours ago"
    elif [[ $hours -gt 0 ]]; then
      commit_age+="${hours} hours $(( minutes % 60 )) minutes ago"
    else
      commit_age="${minutes} minutes ago"
    fi
